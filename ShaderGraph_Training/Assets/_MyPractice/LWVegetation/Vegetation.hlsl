...
float3 VegetationDeformation(float3 position, float3 origin, float3 normal, half leafStiffness, half branchStiffness,
	half phaseOffset, float bendStrength, float windDirectionX, float windDirectionZ, float windFrequency)
{
    ///////Main Bending
    float fBendScale = bendStrength;//main bend opacity
    float fLength = length(position);//distance to origin
    float2 vWind = float2((windDirectionX + sin(_GlobalTime * windFrequency + origin.x + origin.y)) * 0.1, (windDirectionZ + sin(_GlobalTime * windFrequency + origin.z)) * 0.1);//wind direction
    ...
    float2 vWavesIn = _GlobalTime * windFrequency + float2(fVtxPhase, fBranchPhase );
